...
#
# 土曜の 03:00 に週 1 回だけ回す。上場中の全銘柄を 2 秒間隔で叩くので 2 時間 20 分かかる。
# 日足は 1 日 1 本しか増えないため、毎晩やっても取れる量は変わらない。週 26000 リクエストと
# 週 3700 リクエストの差になるので、週次にしている。
#
# 03:00 なのは apt-daily-upgrade.timer を避けるため。あちらは OnCalendar=6:00 に
# RandomizedDelaySec=60m なので 06:00〜07:00 の窓で発火する。05:00 開始だと 07:20 まで
# 走って丸ごと重なる。03:00 なら 05:20 に終わり、40 分空く。遅らせるときはこの窓に
# ぶつからないか確かめること。
#
