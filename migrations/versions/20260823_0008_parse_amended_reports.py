...
動くのは中核の数値になる。純資産・総資産・自己資本比率・EPS・経常利益・ROE が変わり、幅も
小さくない。Abalance は EPS が 53.50 から -17.65 になり、黒字が赤字に反転していた。全 7,150
書類で見れば 1% 前後だが、動くときは判断の土台ごと動く。

訂正報告書は差分ではなく全文になる (585 組すべてでファクト数の比が 0.99〜1.02)。だから元と
マージせず、期ごとに最新の書類を採ればよい。マージはむしろ誤りになる。訂正側で消えるファクト
が 18 組 68 件あり、中身は PER・ROE・希薄化 EPS・配当性向だった。赤字だと PER は算定できず
有報に載らない。訂正で損益が動いた結果として消えているので、元から拾い直すと算定不能な値が
残る。
