...
訂正有価証券報告書 (`130`) はまだ解析していない。ただし**様式は有報と同じで、このパーサで
そのまま読める**。585 件を試して全件が解析できた。
...
`Upper` と `Lower` は予想を幅で出す会社の上下限になる。要素としては全書類に置かれるが、
実測 260 書類ではすべて値が空だった。実際に使う会社は稀で、findocgen がこれを捨てていた
実害は無かった。それでも読めるようにしてある。値が入っていれば拾う。
