...
JPX → EDINET → TDnet の順に回る。所要は 10〜40 分。

| 順 | 処理 | 所要 |
| --- | --- | --- |
| 1 | JPX 銘柄一覧 | 数秒 |
| 2 | EDINET | 1〜5 分 |
| 3 | TDnet | 5〜30 分 (決算期のピークで) |
...
`scripts/nightly.sh` を cron から毎晩叩く。JPX → EDINET → TDnet → 株価 の順に回る。
スクリプトはリポジトリ直下に移動してから実行するので、cron 側で `cd` は要らない。
...
```bash
journalctl -t kabu -n 100
journalctl -t kabu --since "01:00" --until "05:00"
```

01:00 に始めるのは、TDnet の開示が 23:55 まで出るため。日付が変わるまで待たないと当日ぶんを
取りこぼす。逆に遅らせると、株価の 2 時間が朝までに終わらない。
...
途中で 1 つ落ちても後続は走らせる。TDnet は 31 日で消えるので、EDINET の失敗に巻き込まれて
止まるのが一番痛い。失敗があれば終了コード 1 で返すので、cron のログに残る。

多重起動は `flock` で防ぐ。前の晩の実行が終わっていなければ黙って抜ける。systemd timer は
使わない。ログは `logger` で journald に入り、実行順はスクリプトの中で決まる。
