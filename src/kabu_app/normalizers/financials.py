...
そもそも営業利益を出さない会社がある。8,323 書類のうち当期の 6 項目がそろうのは 95.9% で、
欠けの内訳は営業利益 325 件・売上高 21 件・経常利益 5 件だった。営業利益の 325 件は日本 GAAP
225 件、IFRS 88 件、US GAAP 12 件で、銀行と保険が中心になる。**IFRS には営業利益の定義が
無い**ので、開示しない企業がある。税引前利益で代えることはしない。定義の違う値を同じ項目に
入れると、営業利益率が会社ごとに別の意味になるため。

``net_assets`` は日本 GAAP の純資産で、非支配株主持分と新株予約権を含む。IFRS の親会社所有者
帰属持分とは範囲が違う。自己資本が要るなら別項目として足すこと。findocgen はこれを ``Equity``
という名前で持っていて、名前と中身がずれていた。

``unit`` は元のファクトのものをそのまま持つ。ほとんど ``JPY`` になるが、米ドル建てで決算を
出す会社がある。三井海洋開発 (6269) の売上高は ``USD`` で入る。集計するときは ``unit`` で
絞ること。円と混ぜると、その会社だけ桁が 2 つずれる。為替で換算はしない。どのレートを使うか
は分析側の判断で、名寄せの仕事ではないため。
"""
