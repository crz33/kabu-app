...
定性情報 (``XBRLData/Attachment/qualitative.htm``) はまだ取り込んでいない。読むときのために
分かったことを残す。**このファイルに XBRL のタグは付いていない。** ``ix:nonNumeric`` 要素が
1 つも無く、ただの HTML になる。要素名で引くことはできない。

それでも文章の分析には PDF より向く。見出しの階層がそのまま残るため。PDF はレイアウト解析が
要り、表が崩れ、段組みで文の順が入れ替わる。見出しも失われるので「経営成績の概況」と
「業績予想の根拠」を切り分けにくい。

気をつける点が 2 つある。見出しの文言は会社ごとに揺れるので、``①`` や ``（１）`` の形に
頼った切り分けは壊れる。それと表が ``<table>`` で入っているため、素朴にテキスト化すると
数字が文章に混ざる。
"""
...
- ``PL``: 損益計算書
- ``PC``: 損益及び包括利益計算書。包括利益を 1 つの計算書にまとめる方式で出す会社が使う。
  実測 230 書類が該当し、そのすべてで ``ci`` が入っていなかった。``pl`` と ``ci`` を 2 本に
  分ける方式との違いになる。数値は同じ要素名で並ぶので、名寄せのときは ``PL`` に寄せる
- ``CI``: 包括利益計算書。``pl`` と 2 本立てにする方式で出る
- ``CF``: キャッシュ・フロー計算書
- ``SS``: 株主資本等変動計算書
- ``SG``: セグメント情報
...
``Upper`` と ``Lower`` は予想を幅で出す会社の上下限になる。要素としては全書類に置かれ、
使わない会社では空のまま残る。全 26,138 件では 309 書類で実際に値が入っていた。会社予想を
出す 25,350 書類に対して 1.2% になる。

findocgen の正規表現は ``(Result|Forecast)Member$`` で終わっており、この 309 書類の予想を
丸ごと落としていた。少数だが、レンジで出すのは業績の振れが大きい会社になる。捨てると、
そういう会社だけ予想が無い状態になる。
"""
