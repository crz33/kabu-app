...
class EdinetFact(Base, TimestampMixin):
    ...
    値とセグメント別の値が別の行として並ぶ。実データでは営業利益の 7 割の書類でセグメント別
    が同居していた。連結全体だけが欲しいときは ``member IS NULL`` で絞る。これを潰して
    1 行にまとめると、後から区別できなくなる。

    値は円のまま入れる。有報の表は百万円単位で刷られるが、XBRL の中身は円で、桁を丸める
    のは表示側の仕事になる。``decimals`` は原文の精度表示をそのまま残したもので、値の
    スケールとは関係しない。前身の findocgen はここを「百万円」と注記していたが、実際には
    円だった。
    """
