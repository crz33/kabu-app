...
表題に「決算短信」を含む開示はすべて残す。「決算短信の発表日変更のお知らせ」のような短信
そのものでない開示も混じるが、捨てない。

PDF は必ず落とし、XBRL があるときだけ ZIP も落とす。XBRL が付かない短信はごく少ない。実測
では決算短信 24,889 件のうち 47 件 (0.19%) だった。ZIP しか無い開示があると、本文を読むのに
2 系統の処理が要る。同じ会社の短信を時系列で並べたときに読み方が変わるのが一番厄介なので、
どの開示も PDF で読めるようにしてある。数値は ZIP から取り、XBRL の無い 47 件だけは PDF から
読むしかない。PDF は平均 471KB で、年 9GB ほど増える。
"""
