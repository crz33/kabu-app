...
間隔だけでは足りない。締め出しは 1 銘柄あたりのページ数で決まる。2026-08-21 に
560 銘柄 x 32 ページ = 約 18000 リクエストを一度に流そうとして、23 分で締められた。
同じ 2 秒間隔でも、findocgen の週次 (3704 リクエストを 2 時間) は無傷だった。回復には
数時間かかり、その間はブラウザからも返らない。UA や cookie の問題ではない。

遡って取り直すときは銘柄数を区切ること。50 銘柄なら 2000 リクエストを 65 分で、findocgen
の週次と同じ密度に収まる。夜間バッチはこの数で回している。
"""
